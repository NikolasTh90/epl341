...
public class Node {
    ...
    public void generate_children(){
        int right[] = {robot_coordinates[0]+1, robot_coordinates[1]};
        int left[] = {robot_coordinates[0]-1, robot_coordinates[1]};
        int up[] = {robot_coordinates[0], robot_coordinates[1]+1};
        int down[] = {robot_coordinates[0], robot_coordinates[1]-1};
    ...
    private boolean isValidMove(int coordinates[]){
        int maxX = this.grid.grid.length - 1;
        int maxY = this.grid.grid[0].length - 1;
        if(coordinates[0]>maxX || coordinates[1]>maxY)
            return false;
        if(coordinates[0]<0 || coordinates[1]<0)
            return false;
        if(grid.grid[coordinates[1]][coordinates[0]] == 1)
            return false;
        return true;
